if exists (select 1
            from  sysobjects
           where  id = object_id('sp_update_category_layer')
            and   type = 'P')
   drop proc sp_update_category_layer
go

--更新树级层级关系
create  proc [dbo].[sp_update_category_layer]
as
...
with p as (
	...
	from ProCategory a where a.ParentId = 0
	union all
	select a.Id, a.Name, a.OrderNo,

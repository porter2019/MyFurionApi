...
-- 尝试删除函数，如果存在的话
DROP FUNCTION IF EXISTS fn_update_directory_category_layer() CASCADE;

-- 创建函数
CREATE OR REPLACE FUNCTION fn_update_directory_category_layer()
RETURNS void AS $$
BEGIN
    -- 使用递归CTE来构建层级结构并更新
    WITH RECURSIVE p AS (
